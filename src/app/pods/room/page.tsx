...
export default function PodRoomPage() {
  ...
  // Simulate API fetch for pod data
  useEffect(() => {
  ...
  const generateRoadmap = async () => {
    ...
    const phases: RoadmapPhase[] = [
      {
        ...
        weekRange: 'Week 1-2',
        tasks: ['Project initialization', 'Database schema design', 'API structure setup'],
        completed: 0,
        total: 3
      },
      {
        ...
        weekRange: 'Week 3-5',
        tasks: ['User authentication', 'Chat interface', 'Message persistence'],
        completed: 0,
        total: 3
      },
      {
        ...
        weekRange: 'Week 6-8',
        tasks: ['OpenAI API integration', 'Context management', 'Response optimization'],
        completed: 0,
        total: 3
      },
      {
    ...
    // Update progress to 10% and show Generate Tasks button
    if (podData) {
      setPodData({ ...podData, progress: 10 });
    }
  ...
  const generateTasks = async () => {
    ...
    if (podData) {
      ...
      setPodData({
        ...
        progress: 25 // Update progress after tasks are assigned
      });
        ...
        <div className="lg:col-span-2 space-y-6">
            ...
            {roadmapPhases.length === 0 ? (
              <div className="text-center py-8">
                <p className="text-gray-400 mb-4">
                  Let AI create a comprehensive roadmap for your project
                </p>
                <Button
                      ...
                      <div className="space-y-2">
                        {phase.tasks.map((task, taskIndex) => (
                          <div key={taskIndex} className="flex items-center gap-2 text-sm">
                            {taskIndex < phase.completed ? (
                              <CheckCircle2 className="w-4 h-4 text-green-400 flex-shrink-0" />
                            ) : (
                              <Circle className="w-4 h-4 text-gray-500 flex-shrink-0" />
                            )}
                            <span className={taskIndex < phase.completed ? 'text-gray-400 line-through' : 'text-gray-300'}>
                              {task}
                            </span>
                          </div>
                        ))}
            ...
            {!tasksGenerated && !showGenerateTasks && (
              <div className="mb-4 p-3 bg-yellow-500/10 border border-yellow-500/30 rounded-lg">
                <p className="text-xs text-yellow-400">
                  💡 Generate roadmap first, then generate tasks to see all team members with assigned tasks.
                </p>
              </div>
            )}
            ...
            {showGenerateTasks && !tasksGenerated && (
              <div className="mb-4 p-3 bg-indigo-500/10 border border-indigo-500/30 rounded-lg">
                <p className="text-xs text-indigo-400">
                  ✨ Roadmap generated! Click "Generate Tasks" to assign tasks to team members.
                </p>
              </div>
            )}
                    ...
                    {member.tasks.slice(0, 3).map((task) => {
                      ...
                      return (
                        <motion.div
                          ...
                          className="bg-gray-900/50 rounded-lg p-2.5 border border-gray-700/30 hover:border-gray-600/50 transition-colors"
                        >
                          <div className="flex items-start gap-2">
                            {/* Status Dot */}
                            <div className={`w-2 h-2 rounded-full ${statusConfig.dotColor} mt-1.5 flex-shrink-0`} />
                            
                            {/* Task Content */}
                            <div className="flex-1 min-w-0">
                              <div className="flex items-center gap-2 mb-1">
                                <p className={`text-xs font-medium ${task.status === 'done' ? 'line-through text-gray-500' : 'text-gray-300'}`}>
                                  {task.title}
                                </p>
                              </div>
          ...
          >
            ...
            {podData.repo ? (
              <div className="space-y-4">
                <div className="flex items-center justify-between p-4 bg-gray-800/50 rounded-xl">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h3 className="font-medium">{podData.repo.name}</h3>
                      <div className="flex items-center gap-4 mt-1 text-sm text-gray-400">
                        <span className="flex items-center gap-1">
                          <Github className="w-3 h-3" />
                          {podData.repo.commits} commits
                        </span>
                        <span className="flex items-center gap-1">
                          <Star className="w-3 h-3" />
                          {podData.repo.stars} stars
                        </span>
                      </div>
                    </div>
                  </div>
                  <a
                    href={`https://github.com/${podData.repo.name}`}
                    target="_blank"
                    rel="noopener noreferrer"
              ...
              <div className="text-center py-8">
                <p className="text-gray-400 mb-4">No repository linked yet</p>
            ...
            <div className="flex flex-col items-center justify-center py-6">
              <div className="relative w-32 h-32 mb-4">
                ...
                >
                  ...
                  <circle
                    ...
                    strokeDashoffset={`${2 * Math.PI * 56 * (1 - podData.progress / 100)}`}
              ...
              <div className="w-full">
                <div className="h-2 bg-gray-700 rounded-full overflow-hidden">
                  <motion.div
                    initial={{ width: 0 }}
                    animate={{ width: `${podData.progress}%` }}
            ...
            <div className="grid grid-cols-2 gap-4 mb-4">
              {podData.members.map((member) => (
                <motion.div
                  ...
                  transition={{ duration: 0.3 }}
                  className="flex flex-col items-center p-3 bg-gray-800/50 rounded-xl hover:bg-gray-800/70 transition-colors"
                >
